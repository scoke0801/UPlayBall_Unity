...
**현재 모드 범위:** `BaseballManager_PROJECT.md`의 2026-08-31 결정과 43절에 따라 선수 모드와
구단주 모드를 별도 진입점으로 제공한다. 아래의 팀 운영·카드 경제 금지 규칙은 선수 모드의
플레이어 권한에 적용한다. 구단주 모드는 해당 모드의 명시적인 운영 권한과 경제 규약을 따르며,
Core/Simulation 경계·결정론·시뮬레이션과 표현 분리 계약은 두 모드 모두에 적용한다.

구단주 모드의 팀컬러 효과는 플레이어와 AI 구단 모두에 적용한다. AI는 현재 25인 로스터의
발동 후보에서 실제 대상자별 능력치 보너스 합계 순으로 최대 두 슬롯을 선택하고, 동점은 ID로
고정한다. 공통 발동·중첩 규칙을 사용하며 플레이어 상대 경기와 AI 대 AI 경기 및 공개 UI가
같은 선택 결과를 소비한다. 카드 수집·강화 경제는 플레이어 전용이며 선수 커리어와 새 게임 이전
역사 베이크에는 이 AI 팀컬러 정책을 적용하지 않는다.
...
`MatchSimulator`는 공통 `DetailedMatchEngine`의 이닝·수비·주루·기록 규칙을 사용한다.
사용자 승인에 따라 구단주 모드의 **플레이어가 속하지 않은 조**에만 `AggregateBackground` 타석 단위
계산을 사용한다. 플레이어 조 전체와 선수 커리어·역사 베이크는 상세 투구 계산을 유지한다.
간이 계수 변경은 상세 경로 대비 대량 통계·전력별 승률·전체 월드 성능 검증을 동반한다.
과거 경량 경기 구현을 다시 활성화하거나 간이 경로를 다른 범위로 확대하지 않는다.
...
- **콘텐츠(밸런스 수치·선수 데이터·SO 정의 등) 수정으로 세이브 데이터 호환성이 깨지는 것은 현재 전혀 고려 대상이 아니다.** 아직 개발 중이며 디스크 세이브가 없으므로, 콘텐츠를 바꿀 때 기존 세이브와의 하위 호환을 지키려고 마이그레이션을 만들거나 변경을 주저할 필요가 없다. 이 판단은 위 `SaveVersion` 규칙이 실제로 유효해지는 시점(실제 세이브 구현 이후)에 재검토한다.
...
- **향후 배경 제거용 이미지 원본은 단색 크로마키 배경으로 생성한다.** 기본은 균일한 녹색 `#00FF00`이며 전경에 녹색이 있으면 전경과 겹치지 않는 마젠타 `#FF00FF` 등으로 선택하고 프롬프트에 기록한다. 배경의 그라데이션·질감·그림자·체크무늬와 전경에 번지는 배경색은 금지한다. 최종 투명 PNG와 생성 원본을 구분한다. 배경 제거 요청에는 `-Mode ChromaKey`로 변환하고 알파·밝고 어두운 배경 합성까지 검증한다.

- **선수 초상화·유니폼·외형 양산은 `docs/art/PlayerPortraitProduction/README.md`를 따른다.** `references/player-figurine-anchor-v1.png`와 고정 마스터 프롬프트를 사용하고 유니폼·외형은 별도 변주한다. 정면 무동작 프로필과 실제 투명 배경을 유지한다. 이전 화풍을 섞거나 마스터를 임의 변경하지 않는다.

- **이미지 생성 → 배경 제거가 필요하면 `Tools/ImageBackground/Remove-ImageBackground.ps1`을 사용한다.** ImageGen에 투명 배경을 요청했더라도 실제 파일의 알파 채널을 확인한다. 이미지에 그려진 체크무늬는 투명 배경이 아니다.
- 원본을 보존하고 별도 이름의 RGBA PNG로 출력한다. 실행법과 제약은 `Tools/ImageBackground/README.md`를 따른다.
- 크로마키 모드는 내부 틈·분리된 소품과 혼합 경계를 처리한다. 키 색은 전경과 겹치지 않게 선택한다. 기존 무채색 배경은 `-Mode Neutral`과 이미지별 전경 보호 다각형을 사용한다. 복잡한 배경의 의미 기반 분할은 지원하지 않는다.
- 결과는 알파 값과 실제 합성 화면으로 검증한다. 얼굴·의상 손실이나 배경 잔여를 확인하지 않고 완료로 보고하지 않는다.
