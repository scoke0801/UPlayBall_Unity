...
Archive를 익명 Runtime-safe 가상 콘텐츠로 변환한다. Runtime Game Flow에서는 두 도구를 호출하지 않는다.
...
## Runtime-safe 가상 콘텐츠 Bake

정규화 Archive를 실제 이름·구단 식별자가 없는 10개 가상 Franchise의 Baked 콘텐츠로 변환한다.

전체 Archive의 Editor 검수용 에셋은 Runtime `Resources`와 분리된 Unity 공식 Editor 전용 경로에
...
`--verify-editor-assets`는 저장 직후 모든 파일을 다시 읽어 Hash, 10개 Franchise, Core25 구성,
Foreign/중복, 수상 quota를 재검증한다.

산출물에는 공통 `PlayerPerson`/`PlayerSeason`, Normal Card, TeamSeason/Core25,
Runtime-safe Original Record, Stable ID와 `ContentHash`가 들어간다. 같은 입력·버전·Seed는 같은
바이트를 만들며, 연속 연도는 Person 단위로 연결하고 직전 시즌 능력치를 약한 prior로 반영한다.
...
2026-09-02 전체 Archive 검증 결과:

```text
44시즌 / 정규 Franchise 연도별 10개
...
ArchiveHash  dc6e328f8468bd18ae7147eedd53561853703b5f19521019ce70c29dfc1fd030
Asset Files  46 JSON / 12,908,847 bytes
```
