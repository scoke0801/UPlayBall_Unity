...
$source = [Drawing.Bitmap]::FromFile((Join-Path $PSScriptRoot 'connector-transparent.png'))
...
$asset = Join-Path $root 'Assets/10.Datas/Resources/UI/OwnerPowerUp/skill_connector_v1.png'
$texture = [Drawing.Bitmap]::new(256, 80)
$graphics = [Drawing.Graphics]::FromImage($texture)
$graphics.InterpolationMode = [Drawing.Drawing2D.InterpolationMode]::HighQualityBicubic
$graphics.DrawImage($crop, 0, 0, 256, 80)
...
$vertical.Save((Join-Path $root 'Assets/10.Datas/Resources/UI/OwnerPowerUp/skill_connector_vertical_v1.png'), [Drawing.Imaging.ImageFormat]::Png)
    ...
    foreach ($group in $groups) {
        ...
        for ($a=0; $a -lt $group.Count; $a++) {
            for ($b=$a+1; $b -lt $group.Count; $b++) {
                ...
                if ($dx -ne 0) { Draw-Tinted $g $texture ([Drawing.Rectangle]::new($cx-13,$cy-4,26,8)) $color }
                else { Draw-Tinted $g $vertical ([Drawing.Rectangle]::new($cx-4,$cy-13,8,26)) $color }
            }
        }
    }
}
$g.FillRectangle([Drawing.Brushes]::WhiteSmoke, 15, 175, 335, 145)
$g.DrawImage($texture, 45, 215, 270, 84)
$g.DrawImage($texture, 405, 215, 270, 84)
$preview.Save((Join-Path $PSScriptRoot 'connector-review.png'), [Drawing.Imaging.ImageFormat]::Png)
$g.Dispose(); $preview.Dispose(); $tile.Dispose(); $texture.Dispose(); $vertical.Dispose()
Write-Output 'Generated 256x80 connector texture and light/dark + 50px tile composite.'
