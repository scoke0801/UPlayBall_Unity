...
### 2026-09-11 선수 오더 시각·편성 흐름 개정

사용자가 승인한 카드 중심 스포츠 보드 개선에 따라 **선수 오더에 한해** 아래 규칙이 과거 밝은 본문·상단 저장·컨디션 Plot 계약보다 우선한다.

- 공통 Shell과 카드 원화를 유지하고 `CareerUiTheme.RosterBoard/Surface/Text/Accent` 토큰으로 차콜 본문을 구성한다. 이름은 등급별 원화 명찰 안에 배치하고 덮개로 카드 장식을 가리지 않는다. `OwnerUiButtonSkin.SetBoardStyle`은 기존 프레임을 재사용한다.
- 카드 선택은 독립된 블루 테두리와 상단 선택 라벨을 함께 표시하고 선택 해제 시 원래 배치 상태로 복귀한다.
- 상단은 타자/투수·인원·팀컬러 요약·편집 전환에 사용한다. Workspace 안의 고정 하단 `RosterActions`에 저장·변경 취소·검증 결과를 둔다. Shell의 숨겨진 Action Slot을 임의로 켜지 않는다.
- 분석은 컨디션 그래프와 설명을 먼저 표시하고, 그 아래에 수비 경고와 `수비 위치 확인` 행동을 표시한다. 컨디션은 타순·투수 보직별 저장값 0~100을 기존 `UIRosterConditionPlot` 막대 그래프로 표시하고, 배치·궁합 보정 전 값임을 명시한다.
- 선수 교체는 기존 미리보기 계약을 유지한다. 변경 후 우측에서 주 포지션과 실제 카드 능력치를 비교하고 하단 저장으로 확정한다. 상세 정보가 없는 능력치는 대시로 표시한다.
- 보유 목록의 이름 검색은 한글 조합을 보호하도록 입력 완료 시 적용한다. 결과가 없으면 검색·연도·구단·종류·포지션 초기화 행동을 제공한다.
- 배치 상태는 초상 위가 아닌 상단 역할 영역에 한 번만 표시한다. 동일 인물의 다른 카드가 현재 Preview에 배치되어 있으면 `동일 선수 배치 중`을 표시한다. 미배치 문구는 반복하지 않으며 구체적인 교체 안내는 선택 시 제공한다.
- 상단 Toolbar는 80px을 예약하고 카드 비율에서 역할 패널 높이를 계산한다. 2240px을 넘는 본문은 중앙 최대 폭을 사용한다. 필터는 고정하고 카드 목록과 분석 본문은 각각 스크롤한다.
- 검수 명령은 `Tools/RosterUiValidation/Invoke-RosterUiValidation.ps1`이다. 원 프로젝트와 별도 Unity 프로젝트에서 EditMode 회귀와 네 해상도 이미지를 생성한다.
...
- 동일 `PlayerPersonId`의 다른 카드가 현재 Preview에 배치되어 있으면 상단에 `동일 선수 배치 중`을 표시한다. 기존 동일 선수 카드 교체와 조회 모드의 상세 보기는 유지한다.
...
- 선수 오더 컨디션 그래프는 카드와 동일한 `ConditionLevel`을 두 단계씩 묶는다.
  1~2 보라(안좋음), 3~4 파랑(조금 나쁨), 5~6 노랑(평범), 7~8 주황(좋음),
  9~10 빨강(매우 좋음)을 사용한다. 막대 높이는 기본 컨디션 0~100이며 수치와 상태명을 함께 표시한다.
  어두운 보드와 최소 수평 기준선을 사용하고 밝은 구간 배경·세로 격자는 사용하지 않는다.
