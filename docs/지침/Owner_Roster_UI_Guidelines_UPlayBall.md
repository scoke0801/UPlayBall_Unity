...
- 분석은 수비 경고와 `수비 위치 확인` 행동을 먼저 표시한다. 컨디션은 타순·투수 보직별 저장값 0~100을 기존 `UIRosterConditionPlot` 막대 그래프로 표시하고, 배치·궁합 보정 전 값임을 명시한다.
...
- 배치 상태는 초상 위가 아닌 상단 역할 영역에 한 번만 표시한다. 미배치 문구는 반복하지 않으며 동일 선수 교체 안내는 선택 시 제공한다.
