...
> 2026-09-10 사용자 요청 반영: 일반 카드 고코스트 평가를 제한적으로 수정하고 특수 카드
> 593장·142개 레시피를 발급했다. 레전드·커리어하이 영입 UI 및 레어 Scout를 연결했다.
> Cost 10 조건은 유지하며 Cost Bake에서 연도 최고 성과와 검증된 구원 역할의 가격 하한을
> 적용한다. 특수 카드 발급 단계에서는 원본 Cost를 수정하지 않는다. 아래 과거 차단 기록보다
> `docs/reports/고코스트_평가_특수카드_연결.md`의 최신 결과를 우선한다.
