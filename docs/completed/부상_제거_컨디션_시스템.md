# 부상 시스템 제거 + 컨디션 시스템 구현

> 상태: 코드 구현 완료. UI 화면 및 Simulation 실행 검증은 사용자 검증 범위다.
...
## 3. 확정된 설계 결정 (2026-09-06)

1. 기간 결장 이벤트는 완전히 제거한다. 최하 컨디션 고정 같은 우회 결장도 두지 않는다.
2. 자연 변동은 이미 Production에 연결된 경기 출장 소모와 주간 회복 곡선을 유지한다. 새 일별 난수
   곡선은 추가하지 않아 동일 Seed 재현성과 플레이어의 원인 파악을 우선한다.
3. 0~100 `StoredBaseCondition`을 단일 원본으로 사용한다. 경기와 UI의 10단계는
   `ConditionPresentationTable.GetLevel`이 계산하는 파생 표시값이며 별도 상태로 저장하지 않는다.

훈련의 기존 확률 리스크는 기간 결장이 아니라 `GrowthConditionSetbackResult`로 이름과 계약을 바꿨다.
결과는 오직 0~100 컨디션을 추가 차감하며 다음 활동 선택에서 즉시 설명 가능한 비용으로 남는다.
...
1. [x] §3 설계 결정 3가지 확정 (문서화하고 이 TODO에 반영)
2. [x] 0~100 단일 원본과 `ConditionPresentationTable` 10단계 파생 매핑 연결
3. [x] `ConditionChemistryBalanceTable`에 경기 컨디션 능력치 보정 계수 통합
4. [x] 투수 경기 능력치와 출장 소모에 컨디션 연결
5. [x] 타자 경기 능력치와 출장 소모에 컨디션 연결
6. [x] 팀 궁합 → 컨디션 ±1단계 연동
7. [x] 경기 출장 소모·주간 회복 곡선과 설명 가능한 합성 결과 유지
8. [x] `InjuryResolver`/`InjuryState`/`InjuryBalanceTable`/`InjuryResolverTests` 제거
9. [x] `PlayerGrowthState`의 부상 이력·위험 감소 상태 제거, 훈련 리스크를 컨디션 저하로 변환
10. [x] 강제 재활·복귀 보호·은퇴 회고·오프시즌 시장·뉴스 생산 경로 제거
11. [~] 관련 테스트 소스 갱신 및 테스트 어셈블리 컴파일 완료. 실행은 사용자 검증 범위
12. [~] 밸런스 계수를 변경하지 않았으므로 신규 대량 통계는 실행하지 않음. 사용자 검증 범위
