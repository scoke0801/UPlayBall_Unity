...
namespace Baseball.Presentation.SharedScreens
{
    ...
    public static class CareerRecordsSnapshotAdapter
    {
        ...
        public static RecordTableModel CreateLeaderboard(CareerRecordsView view)
        {
            ...
            return LeagueLeaderboardSnapshotAdapter.CreateLeaderboardTable(
            ...
            bool showPrimaryMetricRank)
        {
            ...
            for (int i = 0; i < metrics.Length; i++)
            {
                columns[i + 3] = new RecordTableColumnModel(
                    LeagueLeaderboardSnapshotAdapter.MetricColumnId(metrics[i]),
                    CareerSharedSnapshotFormatters.FormatMetricLabel(metrics[i]),
                    RecordSortValueKind.Number,
                    true,
                    LeagueLeaderboardSnapshotAdapter.PrefersAscendingSort(metrics[i])
                        ? RecordSortDirection.Ascending
            ...
            for (int rowIndex = 0; rowIndex < safeRows.Length; rowIndex++)
            {
                ...
                cells[0] = LeagueLeaderboardSnapshotAdapter.NumberCell(
                    identityColumnId,
                    identityValue.ToString(CultureInfo.InvariantCulture),
                    identityValue);
                cells[1] = LeagueLeaderboardSnapshotAdapter.TextCell(
                    "Team",
                    source.IsCurrent ? source.TeamName + "  (진행 중)" : source.TeamName);
                cells[2] = LeagueLeaderboardSnapshotAdapter.TextCell("League", CareerSharedSnapshotFormatters.FormatLeague(source.LeagueLevel));
                for (int metricIndex = 0; metricIndex < metrics.Length; metricIndex++)
                {
                    CareerRecordMetricValue? metric = LeagueLeaderboardSnapshotAdapter.FindMetric(source.Metrics, metrics[metricIndex]);
                    cells[metricIndex + 3] = metric.HasValue
                        ? LeagueLeaderboardSnapshotAdapter.NumberCell(
                            LeagueLeaderboardSnapshotAdapter.MetricColumnId(metrics[metricIndex]),
                            CareerSharedSnapshotFormatters.FormatMetricValue(metrics[metricIndex], metric.Value.Value),
                            metric.Value.Value)
                        : LeagueLeaderboardSnapshotAdapter.EmptyNumberCell(LeagueLeaderboardSnapshotAdapter.MetricColumnId(metrics[metricIndex]));
                }
            ...
            if (showPrimaryMetricRank && LeagueLeaderboardSnapshotAdapter.ContainsMetric(metrics, view.PrimaryMetric))
            {
                return new RecordTableModel(
                    columns,
                    rows,
                    LeagueLeaderboardSnapshotAdapter.MetricColumnId(view.PrimaryMetric),
                    LeagueLeaderboardService.IsLowerBetter(view.PrimaryMetric)
                        ? RecordSortDirection.Ascending
                        : RecordSortDirection.Descending);
            }
        ...
        {
            return LeagueLeaderboardSnapshotAdapter.FindHighlightedRowId(view.Leaderboard);
        }
    }
}
